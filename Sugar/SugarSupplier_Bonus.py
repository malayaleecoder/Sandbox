...
with open("formatted.txt") as infile, open("distances.txt", "w") as outfile :
    ...
    for line in infile :
        ...
        if data["status"] == "OK" :
            #Find the distance between the two locations using Haversine Formula using the latitudes and longitudes
            #See http://andrew.hedges.name/experiments/haversine
            lat1 = radians(float(data["routes"][0]["legs"][0]["end_location"]["lat"]))
            lat2 = radians(float(data["routes"][0]["legs"][0]["start_location"]["lat"]))
            lng1 = radians(float(data["routes"][0]["legs"][0]["end_location"]["lng"]))  
            lng2 = radians(float(data["routes"][0]["legs"][0]["start_location"]["lng"]))
            ...
            outfile.write(str(i) + " " + str(Radius*c) + "\n")
        else :
...
with open("distances.txt") as infile :
    data_tuple = [tuple(map(float, i.split(" "))) for i in infile]
